...
async function getEmployeesByDesignation(designations) {
  try {
    ...
    console.log(
      `✅ Found ${results.length} employees with designations:`,
      designations,
    );
    return results;
  } catch (error) {
  ...
  for (const employeeId of employeeIds) {
    ...
    if (socketId) {
      try {
        ...
        console.log(
          `✅ Notification sent to ${employeeId} (Socket: ${socketId})`,
        );
        notifiedCount++;
      } catch (error) {
        console.error(
      ...
      console.log(
        `📴 ${employeeId} is not connected (notification saved to DB)`,
      );
    }
...
router.post("/leave-requests", async (req, res) => {
  ...
  try {
   ...
   VALUES (?, ?, ?, ?, ?, ?, ?, 'pending')`,
    ...
    console.log(
      `📢 Notified ${notifiedCount}/${targetEmployees.length} employees online`,
    );
...
router.post("/meeting-requests", async (req, res) => {
  ...
  try {
    ...
    console.log(
      `📢 Notified ${notifiedCount}/${attendees.length} attendees online`,
    );
