...
export default function AdminReportsTable({
  ...
  const [budgetTooltipId, setBudgetTooltipId] = useState(null);
  const budgetTooltipRef = useRef(null);
  ...
  const filteredTasks = flattenedTasks.filter((task) => {
    ...
    if (filterStatus === "all") {
      ...
      matchesStatus = taskProgress === 100 && !isDelayed;
    } else if (filterStatus === "In Progress") {
      matchesStatus =
        endDateTime >= currentIST && taskProgress < 100 && taskProgress > 0;
    } else if(filterStatus==="Not Started"){

       matchesStatus =
        endDateTime >= currentIST && taskProgress < 100 && taskProgress < 1;
  ...
  useEffect(() => {
    const handleClickOutside = (event) => {
      if (
        budgetTooltipRef.current &&
        !budgetTooltipRef.current.contains(event.target)
      ) {
        setBudgetTooltipId(null);
      }
        ...
        const reportsWithBudget =
          data.reports?.map((report) => {
            ...
            if (
              ...
              budgetItems = report.budget;
              budgetTotal = report.budget.reduce(
                (sum, b) => sum + (Number(b.cost) || 0),
                0,
              );
            } else if (task.budget && Array.isArray(task.budget)) {
              const relevantBudgets = task.budget.filter(
                (b) => b.taskReportId === report.reportId,
              );
              budgetItems = relevantBudgets;
              budgetTotal = relevantBudgets.reduce(
                (sum, b) => sum + (Number(b.cost) || 0),
                0,
              );
            }
            ...
            return {
              ...report,
              budgetTotal,
              budgetHistory: budgetItems,
            };
          }) || [];

        setReports(reportsWithBudget);
      } else {
        setReports([]);
      }
    } catch (error) {
                ...
                <div className="absolute right-0 mt-[0.3vw] bg-white border border-gray-300 rounded-lg shadow-lg p-[0.8vw] w-[13vw] z-10">
                  <div className="mb-[0.7vw]">
                    ...
                    >
                      ...
                      <option value="Delayed">Delayed completed</option>
                    ...
                    >
                      ...
                      <td className="px-[0.6vw] py-[0.6vw] border border-gray-300">
                        {(() => {
                          ...
                          let displayStatus = task.status || "Not Started";
            ...
            >
              ...
              <div className="flex-1 px-[1.5vw] py-[1vw]">
                ...
                {loadingReports ? (
                  ...
                  <>
                    <div className="h-[45vh] pr-[0.3vw] overflow-y-auto border border-gray-300 rounded-xl border-separate border-spacing-0 overflow-hidden">
                        ...
                        <tbody>
                          {reports.map((report) => (
                            ...
                            >
                              ...
                              <td className="px-[0.5vw] py-[0.3vw] text-[0.8vw] border-b border-gray-200 relative">
                                <div className="flex items-center gap-[0.5vw]">
                                  <span>
                                    {report.budgetTotal &&
                                    report.budgetTotal > 0
                                      ? `₹${report.budgetTotal.toFixed(2)}`
                                      : "-"}
                                  </span>
                                  {report.budgetHistory &&
                                    report.budgetHistory.length > 0 && (
                                      <div className="relative">
                                        <button
                                          onClick={() =>
                                            setBudgetTooltipId(
                                          ...
                                          className="px-[0.4vw] py-[0.1vw] text-[0.65vw] bg-blue-500 text-white rounded cursor-pointer hover:bg-blue-600"
                                        >
                                          View
                                        </button>
                                        ...
                                        {budgetTooltipId ===
                                          report.reportId && (
                                          <div
                                            ref={budgetTooltipRef}
                                            className="absolute left-0 top-full mt-[0.2vw] bg-white border border-gray-300 rounded-lg shadow-xl p-[0.6vw] w-[15vw] z-50"
                                          >
                                            <div className="max-h-[15vw] overflow-y-auto rounded-lg border border-gray-200 overflow-hidden">
                                              <table className="w-full text-[0.7vw]">
                                                <thead className="bg-gray-50 sticky top-0">
                                                  <tr>
                                                    <th className="px-[0.4vw] py-[0.2vw] text-left border-b">
                                                      Category
                                                    </th>
                                                    <th className="px-[0.4vw] py-[0.2vw] text-right border-b">
                                                      Cost
                                                    </th>
                                                  </tr>
                                                </thead>
                                                <tbody>
                                                  {report.budgetHistory.map(
                                                    (budget, idx) => (
                                                      <tr
                                                        key={idx}
                                                        className="border-b border-gray-200 last:border-0"
                                                      >
                                                        <td className="px-[0.4vw] py-[0.2vw]">
                                                          {budget.costCategory ===
                                                          "Others"
                                                            ? budget.othersName ||
                                                              "Others"
                                                        ...
                                                        <td className="px-[0.4vw] py-[0.2vw] text-right">
                                                          ₹
                                                          {Number(
                                                            budget.cost,
                                                          ).toFixed(2)}
        ...
        >
          ...
          >
            ...
            <div className="space-y-[0.8vw] mb-[1.2vw]">
              <div className="bg-gray-50 rounded-lg px-[0.8vw] py-[0.6vw] border border-gray-200">
                <div className="flex justify-between">
                  <p className="text-[0.75vw] text-gray-600 mb-[0.3vw]">
                    <span className="font-medium">
                      {" "}
                      {selectedReport.activityName ? "Activity :" : "Task :"}
                    </span>{" "}
              ...
              <div className="bg-blue-50 rounded-lg px-[0.8vw] py-[0.6vw] border border-blue-200">
                <p className="text-[0.8vw] font-medium text-gray-900 mb-[0.4vw]">
                  Budgets Added by Employee
                </p>
                <div className="max-h-[10vw] overflow-y-auto">
                  {selectedTask?.budget && selectedTask.budget.length > 0 ? (
                    <table className="w-full text-[0.7vw] border border-blue-200">
                      <thead className="bg-blue-100">
                        <tr>
                          <th className="text-left px-[0.4vw] py-[0.2vw]">
                            Category
                          </th>
                          <th className="text-left px-[0.4vw] py-[0.2vw]">
                            Amount
