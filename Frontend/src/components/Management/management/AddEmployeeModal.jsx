...
const AddEmployee = ({ isOpen, onClose, editingEmployee, onSuccess }) => {
  ...
  useEffect(() => {
    if (editingEmployee) {
      ...
      // Determine if Employee ID can be edited
  ...
  // Check if employment type changed from Intern to On Role
  useEffect(() => {
    if (
      editingEmployee &&
  ...
  // Remove a new file from otherDocs array (before upload)
  ...
  // Delete existing other document from server
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Exit documents (only if status is Inactive)
      if (formData.workingStatus === "Inactive") {
        if (formData.paySlip)
      ...
      <div>
        ...
        {hasExisting && !formData[fieldName] && (
          <div className="mb-[0.5vw] flex items-center gap-[0.5vw] p-[0.5vw] bg-green-50 rounded-lg border border-green-200">
            ...
            <span className="text-[0.85vw] text-green-700 font-medium flex-1">
              Current file exists
            </span>
            <button
              type="button"
              onClick={() => handleViewFile(hasExisting)}
              className="text-[0.85vw] text-blue-600 hover:underline font-medium"
            >
              View
            </button>
            <button
            ...
            <form onSubmit={handleSubmit} className="space-y-[1vw]">
                    ...
                    <div className="grid grid-cols-3 gap-[1vw]">
                      ...
                      <div>
                        ...
                        {!editingEmployee && usernameAvailable === true && (
                          <p className="text-green-600 text-[0.75vw] mt-[0.3vw]">
                            ✓ Available
                          </p>
                        )}
                        {canEditEmployeeId && usernameAvailable === false && (
                          <p className="text-red-500 text-[0.75vw] mt-[0.3vw]">
                            Already taken
                          </p>
                        )}
                        {canEditEmployeeId && usernameAvailable === true && (
                          <p className="text-green-600 text-[0.75vw] mt-[0.3vw]">
                            ✓ Available
                          </p>
                        )}
                        {formData.internId && (
                          <p className="text-gray-600 text-[0.75vw] mt-[0.3vw]">
                            Intern ID: {formData.internId}
                          </p>
                        )}
                        {errors.userName && (
                          <p className="text-red-500 text-[0.75vw] mt-[0.3vw]">
                ...
                {expandedSections.employment && (
                  <div className="p-[1.2vw]">
                    <div className="grid grid-cols-3 gap-[1vw]">
                      ...
                      {/* Exit Documents - Show only when status is Inactive */}
                      {formData.workingStatus === "Inactive" && (
                        <div className="col-span-3 bg-red-50 border border-red-200 rounded-lg p-[1vw]">
                          ...
                          <p className="text-[0.8vw] text-red-700 mb-[1vw]">
                            Please upload exit documents for inactive employees
                          </p>
              ...
              <div className="bg-white border border-gray-200 rounded-lg overflow-hidden shadow-sm">
                ...
                {expandedSections.documents && (
                  <div className="p-[1.2vw]">
                    {/* Profile Picture */}
                    {/* Profile Picture, Resume, and Offer Letter - 3x3 Grid */}
                    ...
                    {/* Resume and Offer Letter - 3x3 Grid */}
